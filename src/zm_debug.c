/*
 * ZoneMinder Debug Implementation, $Date: 2009-02-17 12:26:41 +0000 (Tue, 17 Feb 2009) $, $Revision: 2768 $
...
/**
    ...
    zm_dbg_syslog： 代表系统日志
    zm_dbg_name： 代表当前DEBUG的名称
    zm_dbg_id： 这个Id用于标识不同的DEBUG的信息，相当于数据库里面的自增ID
    zm_dbg_level：调试等级，默认为 INFO
 */
...
/**
  DEBUG 初始化函数
  参数name：zm_dbg_name用于提示该调试信息的名称
  参数id： zm_dbg_id 一般就是个Int整型
  参数level：错误信息的等级
 */
